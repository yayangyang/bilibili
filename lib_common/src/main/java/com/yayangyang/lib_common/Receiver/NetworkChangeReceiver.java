...
import com.yayangyang.lib_common.utils.MyNetWorkUtil;
...
public class NetworkChangeReceiver extends BroadcastReceiver {
    ...
    public List<OnNetWorkChangeListener> mOnNetWorkChangeListeners = new ArrayList<>();
    ...
    public interface OnNetWorkChangeListener {
        void onNetWorkStateChange(String stateInfo);
    }
     ...
     */
    public void setOnNetWorkChangeListener(OnNetWorkChangeListener onNetWorkChange) {
        if (this.mOnNetWorkChangeListeners.contains(onNetWorkChange)) {
            return;
        }
        this.mOnNetWorkChangeListeners.add(onNetWorkChange);
        Log.i("网络状态", "添加一个回调。已设置：" + this.mOnNetWorkChangeListeners.size());
    }
    ...
    public void delOnNetWorkChange(OnNetWorkChangeListener onNetWorkChange) {
        if (this.mOnNetWorkChangeListeners.contains(onNetWorkChange)) {
            this.mOnNetWorkChangeListeners.remove(onNetWorkChange);
            Log.i("网络状态", "删除一个回调。还有：" + this.mOnNetWorkChangeListeners.size());
        }
    }
    ...
    private void setChange(String stateInfo) {
        for (OnNetWorkChangeListener change : mOnNetWorkChangeListeners) {
            change.onNetWorkStateChange(stateInfo);
        }
    }
